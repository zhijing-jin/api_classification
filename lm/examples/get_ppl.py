...
def train():
    ...
    # get ppl
    while not args.do_train:
        raw_text = input("Model prompt >>> ")
        while not raw_text:
            print('Prompt should not be empty!')
            raw_text = input("Model prompt >>> ")
        ppl = get_score(raw_text, tokenizer, model, cuda=not args.no_cuda)
...
def get_score(sentence, tokenizer, model, cuda=True):
    tokenize_input = tokenizer.tokenize(sentence)
    tokens_tensor = torch.tensor([tokenizer.convert_tokens_to_ids(tokenize_input)])
    if cuda:
        tokens_tensor = tokens_tensor.to('cuda')
    with torch.no_grad():
        loss = model(tokens_tensor, masked_lm_labels=tokens_tensor)
        ppl = torch.exp(loss)
        ppl = ppl.item()
    return ppl
